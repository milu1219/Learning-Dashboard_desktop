...
export default function NewSession() {
  ...
  const [form, setForm] = useState({ subject: '', transcript: '' });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const change = (event) => setForm((current) => ({ ...current, [event.target.name]: event.target.value }));
  async function submit(event) {
    event.preventDefault(); setError('');
    if (form.transcript.trim().length < 20) return setError('Enter at least 20 characters of transcript.');
    try {
      setLoading(true);
  ...
  return <div className="max-w-3xl mx-auto"><Link to="/" className="back-link"><ArrowLeft size={16} /> Back to dashboard</Link><div className="mt-5 mb-6"><h1 className="text-3xl font-bold">New Learning Session</h1><p className="text-slate-500 mt-1">Gemini will generate a session title, description, topics, and learning tasks.</p></div>
    <form onSubmit={submit} className="card space-y-5">{error && <div className="alert-error">{error}</div>}<Field label="Subject / Course (optional)"><input name="subject" value={form.subject} onChange={change} placeholder="e.g. Object-Oriented Programming" /></Field><Field label="Transcript"><textarea name="transcript" rows="11" value={form.transcript} onChange={change} placeholder="Paste your transcript or lesson content here…" /></Field><div className="flex justify-end gap-3 pt-4 border-t"><Link to="/" className="button-secondary">Cancel</Link><button disabled={loading} className="button-primary" type="submit">{loading ? <Loader2 className="animate-spin" size={17} /> : <BrainCircuit size={17} />} {loading ? 'Analyzing…' : 'Analyze with Gemini'}</button></div></form>
  </div>;
}
